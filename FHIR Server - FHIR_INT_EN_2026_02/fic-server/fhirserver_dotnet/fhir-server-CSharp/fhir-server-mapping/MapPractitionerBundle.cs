...
namespace fhir_server_mapping
{
    ...
    {
        public static string GetPractitionerBundle(List<fhir_server_entity_model.LegacyPerson> practitioners, string listernerUrl)
        {
            ...
            if (practitioners != null && practitioners.Count > 0)
            {
                ...
                {
                    Uri practitionerUri = new Uri(listernerUrl);

                    entry.Add(new Bundle.EntryComponent()
                    {
                        Resource = MapPractitioner.GetFHIRPractitionerResource(practitioner),
                        FullUrl = $"{practitionerUri.Scheme}://{practitionerUri.Authority}{string.Join("", practitionerUri.Segments)}/{practitioner.PRSN_ID}",
